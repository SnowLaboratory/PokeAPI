        ...
        <tbody class="divide-y divide-gray-400">
            ...
            <slot name="rows">
                <tr v-for="(row, index) in props.rows" class="relative" :class="{
                    ...
                    <slot>
                        <td class="text-left leading-loose" v-show="(columnName in props.columns)"
                            v-for="(value, columnName) in row">{{ value }}</td>
                    </slot>
                        ...
                        <div class="h-8 leading-loose mt-1">
                            <Menu>
                                <MenuButton as="template">
                                    <button
                                        class="border border-gray-500 rounded-lg p-1 hover:bg-slate-100 cursor-pointer relative"
                                        :class="{
                                ...
                                <MenuItems class="absolute inset-x-0 top-full -mt-3 text-sm right-6 left-[unset] z-10 w-24">
                                    <div class=" bg-yellow-50 z-10 border rounded shadow shadow-zinc-300">
                                        <MenuItem v-for="(options, actionName) in props.actions">
                                            ...
                                            <template v-else>
                                                <button @click.prevent="options.action" class="w-full text-left block px-3 py-1 hover:bg-yellow-100">
                                                    {{ actionName }}
                                                </button>
                                            </template>
                                        </MenuItem>
